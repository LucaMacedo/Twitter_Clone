...
from pydantic import (
    BaseModel,
)  # use pydantic to define a model for the product. This is a good practice to ensure that the data is valid and to provide better documentation for the API.
...
):  # use product object instead of dict. This way you can ensure that the data is valid and you can use the product object in the function.
    for index, p in enumerate(products):  # use index to with product
        if p.id == product_id:
            products[index] = (
                product  # update with new product when condition is fullfilled
            )
