...
class ProductCategory(
    str, Enum
):  # enum for product category. This is useful for providing a predefined set of values for the category attribute.
    FOOD = "food"
...
class Product(
    BaseModel
):  # erbt von BaseModel, damit wir die Vorteile von Pydantic nutzen können, wie z.B. die Validierung der Daten und die automatische Generierung von Dokumentation.
    id: int
    name: str = (
        "defaultproduct"  # default value for name. This is useful for testing and for providing default values for the API.
    )
    price: float
    tags: list[str] = []
    description: Optional[str] = (
        None  # optional field, can be None or a string. This is useful for providing additional information about the product without making it mandatory.
    )
...
product = Product(
    id=1, name="Apfel", price=1.99, category=ProductCategory.FOOD
)  # better to use enum values, because it provides better validation and documentation.
product_dict = (
    product.dict()
)  # produkt in dict. Konversion ist nur ein Layer tief. Für mehrere Layer braucht man eine Rekursion.
...
product2 = Product(
    **product_dict
)  # dict in produkt, ** unpackt die Werte aus dem dict und übergibt sie als Argumente an den Konstruktor der Klasse.
print(product2)
