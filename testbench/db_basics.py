...
import bcrypt
from fastapi import (  # FastAPI ist ein modernes Web-Framework für die Erstellung von APIs mit Python. Es basiert auf Starlette und Pydantic und bietet Funktionen wie automatische Generierung von OpenAPI-Dokumentation, Validierung von Anfragen und Antworten sowie Dependency Injection.
    Depends,
    FastAPI,
    HTTPException,
)
from fastapi.security import (  # OAuth2PasswordRequestForm ist eine Klasse, die von FastAPI bereitgestellt wird und zur Verarbeitung von Formularen für die Authentifizierung verwendet wird. Sie enthält Felder wie "username" und "password", die vom Benutzer ausgefüllt werden müssen.
...
from pydantic import (
    EmailStr,
)  # EmailStr ist ein spezieller Datentyp, der von Pydantic bereitgestellt wird und zur Validierung von E-Mail-Adressen verwendet wird. Er stellt sicher, dass die eingegebene E-Mail-Adresse ein gültiges Format hat.
from pydantic import validator  # validere PW

# BaseModel war auch drin, wird ersetzt durch SQLModel
from sqlmodel import (  # SQLModel ist eine Erweiterung von Pydantic, die speziell für die Arbeit mit SQL-Datenbanken entwickelt wurde.
    Field,
    Session,
    SQLModel,
    UniqueConstraint,
...
oath2_schema = OAuth2PasswordBearer(
    tokenUrl="login"
)  # OAuth2PasswordBearer ist eine Klasse, die von FastAPI bereitgestellt wird und zur Implementierung der OAuth2-Authentifizierung verwendet wird. Sie gibt an, dass der Token über den Endpunkt "/login" abgerufen werden kann.
...
def create_db_and_table():
    SQLModel.metadata.create_all(
        engine
    )  # erstellt die Tabelle in der Datenbank, wenn sie noch nicht existiert
...
def create_user(
...
):  # Dependency Injection. FastAPI ruft die get_session-Funktion auf, um eine Session zu erstellen und sie an den Endpunkt weiterzugeben.
    user.password = bcrypt.hashpw(
        user.password.encode("utf-8"), bcrypt.gensalt()
    )  # Passwort wird gehashed, bevor es in der Datenbank gespeichert wird. encode() wandelt den String in Bytes um, da bcrypt nur Bytes akzeptiert.
    ...
    session.refresh(
        new_user
    )  # Aktualisiert das new_user-Objekt mit den Daten aus der Datenbank, einschließlich der automatisch generierten ID.
...
@app.get(
    "/users/", dependencies=[Depends(oath2_schema)]
)  # DP wird nicht gebruacht, weswegen man es in den Decorator schreiben kann
...
def get_current_user(
    session: Session = Depends(get_session), token: str = Depends(oath2_schema)
):
    payload = jwt.decode(token, key=SECRET_KEY)  # dict
    email = payload.get("email")  # email aus token
    current_user = (
        session.query(UserTable).filter(UserTable.email == email).first()
    )  # sucht user in db
    return current_user
